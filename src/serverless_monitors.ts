...
export const HIGH_ERROR_RATE: ServerlessMonitor = {
  ...
  query: (cloudFormationStackId: string, criticalThreshold: number) => {
    return `avg(last_15m):sum:aws.lambda.errors{aws:cloudformation:stack-id:${cloudFormationStackId}} by {functionname,region,aws_account}.as_count() / sum:aws.lambda.invocations{aws:cloudformation:stack-id:${cloudFormationStackId}} by {functionname,region,aws_account}.as_count() >= ${criticalThreshold}`;
  },
  message:
...
export const TIMEOUT: ServerlessMonitor = {
  ...
  threshold: 1,
  query: (cloudFormationStackId: string, criticalThreshold: number) => {
    return `avg(last_15m):sum:aws.lambda.duration.maximum{aws:cloudformation:stack-id:${cloudFormationStackId}} by {aws_account,functionname,region}.as_count() / (sum:aws.lambda.timeout{aws:cloudformation:stack-id:${cloudFormationStackId}} by {aws_account,functionname,region}.as_count() * 1000) >= ${criticalThreshold}`;
  },
  message:
...
export const OUT_OF_MEMORY: ServerlessMonitor = {
  ...
  threshold: 0,
  query: (cloudFormationStackId: string, criticalThreshold: number) => {
    return `avg(last_15m):sum:aws.lambda.enhanced.out_of_memory{aws:cloudformation:stack-id:${cloudFormationStackId}} by {aws_account,functionname,region} > ${criticalThreshold}`;
  },
  message:
...
export const HIGH_ITERATOR_AGE: ServerlessMonitor = {
  ...
  threshold: 86400,
  query: (cloudFormationStackId: string, criticalThreshold: number) => {
    return `avg(last_15m):min:aws.lambda.iterator_age.maximum{aws:cloudformation:stack-id:${cloudFormationStackId}} by {aws_account,region,functionname} >= ${criticalThreshold}`;
  },
  message:
...
export const HIGH_COLD_START_RATE: ServerlessMonitor = {
  ...
  threshold: 0.01,
  query: (cloudFormationStackId: string, criticalThreshold: number) => {
    return `avg(last_15m):sum:aws.lambda.enhanced.invocations{cold_start:true AND aws:cloudformation:stack-id:${cloudFormationStackId}} by {aws_account,functionname,region}.as_count() / sum:aws.lambda.enhanced.invocations{aws:cloudformation:stack-id:${cloudFormationStackId}} by {aws_account,functionname,region}.as_count() >= ${criticalThreshold}`;
  },
  message:
...
export const HIGH_THROTTLES: ServerlessMonitor = {
  ...
  threshold: 0.1,
  query: (cloudFormationStackId: string, criticalThreshold: number) => {
    return `avg(last_15m):sum:aws.lambda.throttles {aws:cloudformation:stack-id:${cloudFormationStackId}} by {aws_account,region,functionname}.as_count() / ( sum:aws.lambda.throttles {aws:cloudformation:stack-id:${cloudFormationStackId}} by {aws_account,region,functionname}.as_count() + sum:aws.lambda.invocations{aws:cloudformation:stack-id:${cloudFormationStackId}} by {aws_account,region,functionname}.as_count() ) >= ${criticalThreshold}`;
  },
  message:
...
export const INCREASED_COST: ServerlessMonitor = {
  ...
  threshold: 0.2,
  query: (cloudFormationStackId: string, criticalThreshold: number) => {
    return `pct_change(avg(last_5m),last_5m):avg:aws.lambda.enhanced.estimated_cost{aws:cloudformation:stack-id:${cloudFormationStackId}} > ${criticalThreshold}`;
  },
  message:
